...
found:
  ...
  // add pid
  struct usyscall *rn_page = (struct usyscall *)USYSCALL;
  rn_page->pid = p->pid;
...
pagetable_t proc_pagetable(struct proc *p) {
  ...
  // map read-only page under trapframe
  void *syscall_page = kalloc();
  if (syscall_page == 0) {
    uvmunmap(pagetable, TRAMPOLINE, 1, 0);
    uvmfree(pagetable, 0);
    return 0;
  ...
  if (mappages(pagetable, USYSCALL, PGSIZE, (uint64)(syscall_page),
               PTE_R | PTE_U) < 0) {
    uvmunmap(pagetable, TRAMPOLINE, 1, 0);
    uvmfree(pagetable, 0);
    kfree(syscall_page);
