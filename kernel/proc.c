...
uint64 mmap(uint64 length, int prot, int flags, int fd) {
  ...
  for (int i = 0; i < 14; i++) {
    ...
    } else if (t_v->addr < lowest_valid) {
      lowest_valid = t_v->addr;
    }
  }

  // assert chosen_v valid
  chosen_v->addr = lowest_valid - length;
  chosen_v->valid = 1;
  chosen_v->len = length;
  chosen_v->prot = prot;
  chosen_v->flags = flags;
  chosen_v->file = file;

  filedup(file);

  return chosen_v->addr;
}